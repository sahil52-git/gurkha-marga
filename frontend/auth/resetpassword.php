<?php
// frontend/auth/resetpassword.php
session_start();

// ── CRITICAL FIX: use the shared database helpers, NOT a local getDB() ───
...
if ($token && $email) {
    try {
        ...
        error_log('resetpassword.php token check: ' . $e->getMessage());
        $alertMsg  = 'A database error occurred. Please try again.';
        $alertType = 'error';
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $tokenOk) {
    ...
    if (strlen($password) < 8) {
        ...
        try {
            ...
            error_log('resetpassword.php update: ' . $e->getMessage());
...
<head>
    ...
    <style>
        *,*::before,*::after{margin:0;padding:0;box-sizing:border-box;}
        ...
        input[type="password"]{width:100%;padding:13px 14px 13px 42px;background:rgba(255,255,255,0.07);border:1px solid rgba(255,255,255,0.14);border-radius:10px;color:#fff;font-size:15px;outline:none;transition:border-color .25s,box-shadow .25s;}
        input[type="password"]::placeholder{color:rgba(255,255,255,0.28);}
        input[type="password"]:focus{border-color:#e94560;background:rgba(233,69,96,0.07);box-shadow:0 0 0 3px rgba(233,69,96,0.18);}
...
<div class="card">
    ...
    <p class="text-center muted"><a href="login.php" class="link">Back to Login</a></p>
</div>
